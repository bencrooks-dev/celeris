...
| **v0.2** | Typed IR + C++ source-gen | IR → C++ emitter, runtime `clang++ -O3` compile, `ctypes` load and marshaling, on-disk per-kernel cache (`~/.celeris_cache/`). First real speedups. |
...
| **v0.5** | Tensor memory model | Multi-dimensional arrays, strides, and basic slicing; promotes the array markers beyond 1-D and lays groundwork for fusion/tiling passes. |
| **v1.0** | Stable kernel compiler | Stabilized IR schema and public API, the golden-kernel registry as a documented extension point, loop fusion/tiling optimization passes, and a published, semver-guaranteed release. |
